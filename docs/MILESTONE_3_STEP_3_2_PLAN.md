...
| **4b. Response composer** | `ResponseComposer` as a response-lifecycle state machine; platform conversation intents (section 8.5); product-defined identity and capability replies; proposed / confirmed / executed / failed / cancelled wording | Wording tests per lifecycle state; no unsupported completion claims; `KnowledgeLookup` honest fallback |
